...
def expanded_conv(input_tensor,
  ...
  with tf.variable_scope(scope, default_name='expanded_conv') as s, \
    ...
    if depthwise_location == 'input':
      ...
      if kernel_size == (2, 2) and padding == "VALID" and stride == 1:
          net = tf.pad(net, [[0,0],[1,1],[1,1],[0,0]])
          net = depthwise_func(net, activation_fn=None, scope='depthwise_1')
          net = depthwise_func(net, activation_fn=None, scope='depthwise_2')
      else:
          net = depthwise_func(net, activation_fn=None)
      ###
    ...
    if depthwise_location == 'expansion':
      ...
      if kernel_size == (2, 2) and padding == "VALID" and stride == 1:
          net = tf.pad(net, [[0,0],[1,1],[1,1],[0,0]])
          net = depthwise_func(net, scope='depthwise_1')
          net = depthwise_func(net, scope='depthwise_2')
      else:
          net = depthwise_func(net)
      ###
...
if __name__ == '__main__':
    import numpy as np
    input = np.random.random([2,224,224,3])
    input = tf.placeholder(tf.float32, [None, 224,224,3])
    output = expanded_conv(input_tensor=input,
                           num_outputs=8,
                           kernel_size=(2,2),
                           padding="VALID")
    print(output)
    with tf.Session() as sess:
        ...
        output_graph_def = tf.graph_util.convert_variables_to_constants(
            sess, input_graph_def, "expanded_conv/output".split(","))

        with tf.gfile.GFile('./expanded_conv.pb', "wb") as f:
